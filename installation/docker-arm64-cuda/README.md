...
```bash
./template.sh push_generic docker.io/docker-username/ee628
```
...
> (e.g., `docker.io/docker-username/ee628`)
...
./template.sh run python -m ee628.template_experiment some_arg=some_value
...
E.g., you can mount it at `/project/ee628` and specify `PROJECT_ROOT_AT=/project/ee628`.
...
export PROJECT_ROOT_AT=/project/ee628
apptainer run \
    -c \
    -B $(pwd):${PROJECT_ROOT_AT} \
    --cwd ${PROJECT_ROOT_AT} \
    --env PROJECT_ROOT_AT=${PROJECT_ROOT_AT} \
    --env PROJECT_NAME=ee628 \
    --env PACKAGE_NAME=ee628 \
    --env WANDB_API_KEY="" \
    --nv ee628_arm64-cuda-root-latest.sif
