...
for i in range(accelerator.gradient_accumulation_steps):
    data.extend(
        [[float(3), float(4)], [float(5), float(12)]] * (accelerator.num_processes // 2)
    )
...
for batch in dataloader:
    ...
    if accelerator.process_index in [0, 1]:
        ...
        print()
        print("params: ", list(model.parameters())[0].data)
        print("data: ", x)
        print("grad: ", model._get_gradients_for_reduction())
        print("global grad norm: ", model.get_global_grad_norm())
...
if accelerator.process_index in [0]:
    ...
    print(
        "scale ratio: ",
        -applied_gradient.cpu() / dataset.select(range(2))["input"].mean(0),
    )
